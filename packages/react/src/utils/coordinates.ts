...
export const fromCartesianPoint = (
  width: number,
  height: number,
  x: number,
  y: number
...
export const fromCanvasCartesianPoint = (x: number, y: number) =>
  fromCartesianPoint(CANVAS_SIZE, CANVAS_SIZE, x, y)

/** Resolves an absolute coordinate point to a cartesian coordinate, relative to the canvas size */
export const toCanvasCartesianPoint = (x: number, y: number) =>
  toCartesianPoint(CANVAS_SIZE, CANVAS_SIZE, x, y)
